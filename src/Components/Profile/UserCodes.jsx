...
const UserCodes = ({ isLoggedIn, useremail }) => {
    ...
    useEffect(() => {
        const fetchCodes = async () => {
            ...
            try {
                // Assuming the backend expects the user's email
                const response = await axios.get(`http://localhost:3000/codes/${useremail}`);
        ...
        fetchCodes();
    }, [useremail]);  // Use useremail as a dependency
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {codes.length > 0 ? (
                        codes.map((code) => (
                            <div 
                                key={code._id} 
                                onClick={() => handleOpenSnippet(code.codeSnippet)} 
